...
if(isset($_GET['ma']) && $_GET['ma'] !=''){
	...
	$tin = $dt_loai_san_pham->doc_thong_tin($_GET['ma']);
	//echo '<pre>',print_r($_POST),'</pre';
	if(isset($_POST) && isset($_POST['luu']) && $_POST['luu']=='luudong'){
		if($dt_loai_san_pham->sua($_GET['ma'],$_POST))
		{
			echo '<script>document.location="main.php?views=typenew";</script>';
		}else
		{
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-edit"></i> Cập nhật nhóm sản phẩm <?php echo '- ',$tin->ten ?></h2>
            ...
            <div class="box-content">
                ...
                <form role="form" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        <label for="ten_loai_san_pham">Tên</label>
                        <input type="text" class="form-control" id="ten_loai_san_pham" onblur="loai_bo_dau('ten_loai_san_pham','alias')" value="<?php echo $tin->ten ?>" name="ten" placeholder="Nhập tên sản phẩm">
                    </div>
					<div class="form-group">
					...
					<!--<div class="control-group">
						...
						<div class="controls">
                            <select name="danhmuccha" id="danhmuccha" data-rel="chosen">
								<option value="0">Chọn nhóm</option>	
								<?php foreach($dsloaicha as $loai){ ?>
								<option  <?php echo ($loai->ma==$tin->danhmuccha)?'selected':''  ?> value="<?php echo $loai->id ?>"><?php echo $loai->ten ?></option>		
								<?php } ?>								
							</select>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <textarea  class="form-control" id="tom_tat" name="noi_dung_tom_tat"><?php echo $tin->noi_dung_tom_tat ?></textarea>
                    </div>-->
					<div class="form-group">
                        <label for="mo_ta_tom_tat">Mô tả</label>
						<textarea  class="form-control" id="mo_ta_tom_tat" name="mota"><?php echo $tin->mota ?></textarea>
						<script type="text/javascript">CKEDITOR.replace('mo_ta_tom_tat'); </script>
                    </div>
                    <div class="form-group">
                        <label for="hinh">Hình đại diện</label>
						<input type="text"  size="48" name="hinhdaidien" value="<?php echo $tin->hinhdaidien ?>"  id="hinh" /><button type="button" onclick="openPopup()">Chọn hình</button>
					...
					<div class="form-group">
                        ...
                        <input type="text" class="form-control" id="tieude" value="<?php echo $tin->tieude ?>" name="tieude" placeholder="Nhập tiêu đề trang">
                    </div>		
					<div class="form-group">
					...
					<div class="form-group">
                        ...
                        <input type="text" class="form-control" id="thutu" value="<?php echo $tin->thutu ?>" name="thutu" placeholder="Thứ tự hiển thị">
                    </div>	
