...
if(!isset($_GET['ma']) || $_GET['ma'] ==''){
	...
	if(isset($_POST) && isset($_POST['luu']) && $_POST['luu']=='luudong'){
		if($xl_menu->them($_POST))
		{
			echo '<script>document.location="main.php?views=menu";</script>';
		}else
		{
			$thongbao='Xảy ra lỗi trong quá trình thêm';
		}	
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-edit"></i> Thêm mới menu </h2>
            ...
            <div class="box-content">
                ...
                <form role="form" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="tieu_de" value="" name="ten" placeholder="Nhập tên menu">
                    </div>
					<div class="form-group">
						...
						?>
                        <label><input type="radio" value="<?php echo $icon ?>" name="icon">
					...
					<div class="control-group">
						...
						<div class="controls">
                            <select name="loai_menu" onchange="an_hien_control(this.value)" id="loai_menu" data-rel="chosen">		
								...
								<option <?php echo ($key=='link')?'selected':'' ?>  value="<?php echo $key ?>"><?php echo $ten ?></option>		
								<?php } ?>								
							</select>
                        </div>
                    </div> 
					...
					<div id="khungloaitintuc" class="control-group">
						...
						<div class="controls">
                            <select onchange="taourl(this)" data-type="danhsachtin" class="form-control" name="id_loai_tin" id="id_loai_tin" data-rel="chosen">
								...
								?>
								<option value="<?php echo $loai->duongdan.'*'.$loai->ma ?>"><?php echo $loai->ten ?></option>		
									<?php 
										...
										if($loaicon)
										{
											...
											?>
											<option   value="<?php echo $con->duongdan.'*'.$con->ma ?>"><?php echo '|--'.$con->ten ?></option>		
												<?php 
												...
												if($loaicon2)
												{
													...
													?>
													<option   value="<?php echo $con2->duongdan.'*'.$con2->ma ?>"><?php echo '|----'.$con2->ten ?></option>		
													<?php
													}
												}
					...
					<div id="khungtintuc" class="control-group">
						...
						<div class="controls">
                            <select onchange="taourl(this)" data-type="chitiettin"   class="form-control" name="id_tin" id="id_tin" data-rel="chosen">
								...
								foreach($dstin as $sp){
								?>
								<option value="<?php echo $sp->duongdan.'*'.$sp->ma ?>"><?php echo $sp->ten ?></option>		
									<?php
									} ?>								
							</select>
                        </div>
					...
					<div class="form-group">
                        ...
                        <input readonly type="text" class="form-control" id="duongdan" value="" name="duongdan" placeholder="Nhập đường dẫn rút gọn SEO">
                    </div>
					<div class="form-group">
                        <label for="link">Liên kết gốc</label>
                        <input readonly type="text" class="form-control" id="link" value="" name="link" placeholder="Liên kết gốc">
                    </div>
					<div class="control-group">
						...
						<div class="controls">
                            <select name="menucha" id="menu_cha" data-rel="chosen">
								...
								if($dsmenucon)
								{
									...
									{
										?>
										<option  value="<?php echo $mncon->ma ?>">|--<?php echo $mncon->ten ?></option>		
										<?php
									}
								}
							 } ?>								
					...
					<div class="form-group">
                        ...
                        <input type="text" class="form-control" id="mota" value="" name="mota" placeholder="Nhập ghi chú cho menu">
                    </div>
					<div class="form-group">
                        <label for="sap_xep">Thứ tự</label>
                        <input type="text" class="form-control" id="sap_xep" value="1" name="thutu" placeholder="Nhập thứ tự hiển thị">
                    </div>
                    <div class="checkbox">
                        <label>
						...
						<label>
                            <input type="checkbox"   value="1" name="macdinh"> Mặc định
                        </label>						
                    </div>
